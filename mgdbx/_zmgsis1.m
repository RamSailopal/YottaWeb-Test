%zmgsis1 ;(CM) Service Integration - Child Process
 ;
 ;  ----------------------------------------------------------------------------
 ;  | %zmgsis1                                                                  |
...
getzv() ; Get $ZV
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":getzve^%zmgsis1"
 q $zyrelease
getzve ; Error
 q $zv
 ;
...
getmsl() ; Get maximum string length
 n max
 new $ztrap set $ztrap="zgoto "_$zlevel_":getmsle^%zmgsis1"
 q 1048576
 i $$isydb() q 1048576
 i $$ism21() q 1023
 i $$ismsm() q 250
...
getmslx() ; Get maximum string length the hard way
 n x,max
 new $ztrap set $ztrap="zgoto "_$zlevel_":getmslxe^%zmgsis1"
...
xinetd ; someone is sure to use this label
 new $ztrap set $ztrap="zgoto "_$zlevel_":inetde^%zmgsis1"
 d child(0,0,"","")
 q
inetde ; error
 w $$error()
...
ifc(ctx,request,param) ; entry point from fixed binding
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":ifce^%zmgsis1"
 i param["$zv" q $zv
...
child(pport,port,tls,opts) ; child
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":childe^%zmgsis1"
...
child4 ; request header received
 ...
 i $e(buf,1,4)="dbx1" d dbxnet^%zmgsis1(.%zcs,buf) d halt(.%zcs)
 s type=0,byref=0 d req2(.%zcs,.req,argc,type,byref,buf,.cmnd,.pcmnd,.var) s @var=buf
 s cmnd=$p(buf,"^",2)
 s hlen=$l(buf),clen=0
 i cmnd="P" s clen=$$dsize($e(buf,hlen-(5-1),hlen),5,62)
...
array2(%zcs,req,argc,xkey,kn,sn,val) ; read array - set a single node
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":array2e^%zmgsis1"
...
uci(uci) ; change namespace/uci
 n x
 new $ztrap set $ztrap="zgoto "_$zlevel_":ucie^%zmgsis1"
 i uci="" q
 s $zg=uci
 q
ucie ; error
 i $g(^%zmgsi1("loglevel"))>0 d event("uci error: "_uci_" : "_$$error())
 q
 ;
getuci() ; get namespace/uci
 n uci
 new $ztrap set $ztrap="zgoto "_$zlevel_":getucie^%zmgsis1"
 s uci=$zg
 q uci
getucie ; error
 q ""
...
mcom(%zcs,req,argc,cmnd,%oref) ; execute M command
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":mcome^%zmgsis1"
 s %zcs("buffer")="",res=""
 ; check options mask
...
resa(%zcs,req,argc) ; return array
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":resae^%zmgsis1"
 s byref=0
...
setio(tblname) ; set i/o translation
 new $ztrap set $ztrap="zgoto "_$zlevel_":setioe^%zmgsis1"
 q ""
...
setunpw(username,password)
 s ^%zmgsi1(0,"username")=username
 s ^%zmgsi1(0,"password")=password
 q 1
 ;
getunpw(username,password)
 s username=$g(^%zmgsi1(0,"username"))
 s password=$g(^%zmgsi1(0,"password"))
 q 1
 ;
...
dbx(%zcs,ctx,cmnd,data,len,param) ; entry point from fixed binding
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":dbxe^%zmgsis1"
 ...
 s type=1 ; string
 i $g(%r(1))="dbxweb^%zmgsis1",res=$c(255,255,255,255) q res
...
dbxcmnd(%r,%oref,cmnd,res,utf16) ; Execute command
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":dbxcmnde^%zmgsis1"
 s res=""
 ; check options mask
 i $d(%zcs(0)) s ok=1 d  i 'ok q 0
 . i $g(%zcs(0,1))=0,(((cmnd>10)&(cmnd<30))!((cmnd>100)&(cmnd<300))) s ok=0 q
 . i $g(%zcs(0,2))=0,cmnd=31,$g(%r(1))'="dbxweb^%zmgsis1" s ok=0 q
 . i $g(%zcs(0,3))=0,((cmnd>40)&(cmnd<60)) s ok=0 q
 . i $g(%zcs(0,4))=0,cmnd=31,$g(%r(1))="dbxweb^%zmgsis1" s ok=0 q
 . q
...
dbxweb(ctx,data,param) ; mg_web function invocation
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":dbxwebe^%zmgsis1"
 s no=0
...
dbxweblr(%cgi,%var,%sys) ; read long request payload
 ...
 new $ztrap set $ztrap="zgoto "_$zlevel_":dbxweblre^%zmgsis1"
...
mgweb(%cgi,%var,%sys)
 ...
 s content=content_"%zmgsi1: v"_$p($$v(),".",1,3)_" ("_$zd(+$h,2)_")"_$c(13,10)
...
mgwebsock(%cgi,%var,%sys)
 n (%cgi,%var,%sys)
 new $ztrap set $ztrap="zgoto "_$zlevel_":mgwebsocke^%zmgsis1"
 s @("res="_"$$"_%sys("wsfunction")_"(.%cgi,.%var,.%sys)")
 d flush^%zmgsis1
 h
mgwebsocke ; Error
 s res="HTTP/2 200 OK"_$c(13,10)_"Error: "_$$error()_$c(13,10,13,10)
 w $$esize256^%zmgsis1($l(res)+5)_$c(0)_$$esize256^%zmgsis1(($g(%sys("no"))+0))_$c(0)_res d flush^%zmgsis1
 h
 ;
...
urld(%val) ; URL decode (unescape)
 new $ztrap set $ztrap="zgoto "_$zlevel_":urlde^%zmgsis1"
 q $$urldx(%val)
urlde ; error
 q $$urldx(%val)
 ;
...
urle(%val) ; URL encode (escape)
 new $ztrap set $ztrap="zgoto "_$zlevel_":urlee^%zmgsis1"
 q $$urlex(%val)
urlee ; error
 q $$urlex(%val)
 ;
...
odbcdt(x) ; Translate ODBC data type code to readable form
 ...
 ;
 ; s x=$$sqleisc^%zmgsis1(0,"SELECT * FROM SQLUser.customer","")
sqleisc(id,sql,params) ; Execute InterSystems SQL query
 new $ztrap set $ztrap="zgoto "_$zlevel_":sqleisce^%zmgsis1"
...
sqleisce1 ; SQL error
 ...
 q $$esize256($l(result))_result
 ;
 ; s x=$$sqlemg^%zmgsis1(0,"select * from patient","")
sqlemg(id,sql,params) ; Execute MGSQL SQL query
 new $ztrap set $ztrap="zgoto "_$zlevel_":sqlemge^%zmgsis1"
...
sqlmgv() ; get MGSQL version
 new $ztrap set $ztrap="zgoto "_$zlevel_":sqlmgve^%zmgsis1"
 s v=$$v^%mgsql()
 q v
sqlmgve ; M error
 q ""
...
event(text) ; log m-side event
 n i,x,y,n,emax
 new $ztrap set $ztrap="zgoto "_$zlevel_":evente^%zmgsis1"
 f i=1:1 s x=$e(text,i) q:x=""  s y=$s(x=$c(13):"\r",x=$c(10):"\n",1:"") i y'="" s $e(text,i)=y
 s emax=100 ; maximum log size (no. messages)
 l +^%zmgsi1("log")
 s n=$g(^%zmgsi1("log")) i n="" s n=0
 s n=n+1,^%zmgsi1("log")=n
 l -^%zmgsi1("log")
 s ^%zmgsi1("log",n,0)=$$head(),^%zmgsi1("log",n,1)=text
 f n=n-emax:-1 q:'$d(^%zmgsi1("log",n))  k ^(n)
 q
evente ; error
 q
 ;
ddate(date) ; decode m date
 new $ztrap set $ztrap="zgoto "_$zlevel_":ddatee^%zmgsis1"
 q $zd(date,2)
...
head() ; format header record
 n uci
 new $ztrap set $ztrap="zgoto "_$zlevel_":heade^%zmgsis1"
 s uci=$$getuci()
heade ; error
